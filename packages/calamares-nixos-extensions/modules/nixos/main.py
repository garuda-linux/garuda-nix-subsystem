...
def run():
    ...
    # Garuda Nix Subsystem: install this flake instead of the generated config.
    # Keeps hardware-configuration.nix, replaces configuration.nix with the
    # subsystem template and adds flake.nix. The flavour picker selects
    # mokka (catppuccin) or dr460nized.
    hostname = variables.get("hostname", "nixos")
    garuda_flake = os.environ.get(
        "GNS_FLAKE_REF", "gitlab:garuda-linux/garuda-nix-subsystem/stable"
    )
    flake_nix = """{{
    ...
    outputs = {{{{ self, garuda }}}}:
        ...
        in
        {{{{
            nixosConfigurations.{host} = garuda.lib.garudaSystem {{{{
                inherit system;
                modules = [ ./configuration.nix ];
            }}}};
        }}}};
}}}}
""".format(flake=garuda_flake, host=hostname)
    libcalamares.utils.host_env_process_output(
        ["cp", "/dev/stdin", os.path.join(root_mount_point, "etc/nixos/flake.nix")],
        None,
        flake_nix,
    )
    packagechooser = gs.value("packagechooser_packagechooser")
    garuda_flavor = (
        "  garuda.catppuccin.enable = true;"
        if packagechooser == "mokka"
        else "  garuda.dr460nized.enable = true;"
    )
    garuda_cfg = (
        cfghead
        ...
        + '  system.stateVersion = "' + variables.get("nixosversion", "26.05") + '";\n'
        + "}\n"
    )
    libcalamares.utils.host_env_process_output(
        ["cp", "/dev/stdin", config], None, garuda_cfg
    )
